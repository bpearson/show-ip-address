...
function _getIPDetails(ipAddr, callback) {
  ...
  var request = Soup.Message.new('GET', 'http://ipinfo.io/' + ipAddr + '/json');
...
function _getIP(callback) {
  ...
  _httpSession.queue_message(request, function(_httpSession, message) {
    ...
    var ipAddrData = JSON.parse(request.response_body.data);
    callback(null, ipAddrData.ip);
  });
}
...
const IPMenu = new Lang.Class({ //menu bar item
  ...
  _init: function() {
    ...
    this._icon = new St.Icon({
      gicon: Gio.icon_new_for_string(Me.path + '/icons/flags/AU.png'),
      icon_size: ICON_SIZE
    });
